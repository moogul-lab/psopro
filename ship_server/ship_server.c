...
void FixItem (ITEM* i )
{
	...
	if (i->data[0] == 2) // Mag
	{
		...
		{
			// Mag fails IRL, initialize it GAY
			m->defense = 500;
			m->power = 0;
			m->dex = 0;
			m->mind = 0;
		...
		case 0x01:
			{
				...
				for (ch=0;ch<serverNumConnections;ch++)
				{
					...
					if ((connections[connectNum]->plySockfd == sockfd) && (connections[connectNum]->guildcard == guildcard))
					{
						...
						//baseTP= *(unsigned short*) &startingData[(client->character._class * 14) + 12];
						...
						for (ch2=0;ch2<client->character.level;ch2++)
						{
							...
							//baseTP += playerLevelData[client->character._class][ch2].TP;
						}
						...
						client->matuse[5] = (client->character.HP - baseHP) / 2;
						//client->matuse[6] = (client->character.TP - baseTP) / 2;

						//client->character.lang = 0x00;

						cd = (unsigned char*) &client->character.packetSize;

						cd[(6*28) + 0x0F] = client->matuse[0];
						cd[(7*28) + 0x0F] = client->matuse[1];
						cd[(8*28)+0x0F]  = client->matuse[2];
						cd[(9*28)+0x0F]  = client->matuse[3];
						cd[(10*28)+0x0F] = client->matuse[4];
						//cd[(5*28)+0x0F] = client->matuse[5];
						//cd[(4*28)+0x0F] = client->matuse[6];
		...
		case 0x03:
			...
			case 0x0B:
				...
				case 0x02:
					...
					break;
				case 0x03:
					if ( ( client->character.HPmat < 500 ) && ( HPMatUse < 500 ) )
						client->character.HPmat += 2;
					else
						mat_exceed = 1;
					break;
				case 0x04:
					if ( ( client->character.TPmat < 1000 ) && ( HPMatUse < 1000 ) )
			...
			case 0x0C:
				...
				case 0x05: // Heart of Chao
					...
					break;
				case 0x06: // Gayhoo! Mag
					if (eq_mag != -1)
						client->character.inventory[eq_mag].item.data[1] = 0x4A;
					break;
				}
				break;
			case 0x0E:
		...
		if ( !rare )
		{
			...
			if ( ( r > ptd->element_probability[area] ) && ( ptd->element_ranking[area] ) )
			{
				...
				while (i->item.data[4] == 0xFF) // give a special
				{
					...
					switch (i->item.data[4])	// nonrare element specials can only tek up to Fire and Thunder
					{
					case 0x19:
						i->item.data[4] = 0x17;
						break;
					...
					/*if (i->item.data[4] == 0x19 || i->item.data[4] == 0x1A || i->item.data[4] == 0x1D || i->item.data[4] == 0x1E)
					{
						i->item.data[4] -= 2; m gud XD m gud success m beri gud sexcess LOL
					}*/
				}
			}
			else
	...
	*(unsigned *) &chatBuf[0x0C] = client->guildcard;
	...
	if ((*(n+1) == 0x002F) && (*(n+2) != 0x002F))
	{
		...
		if ( commandLen )
		{
				...
				if ( client->lobbyNum < 0x10 )
					...
					if ( l->floor[client->clientID] == 0 )
						...
						else
						{
							...
							{
								// Do the level up!!! gay sodaboy

								if (target > 200)
								{
									SendB0 ("Gay levels over 200 not working for now", client);
									/*if (client->character.level < 200) // LOL
									{
										//AddExp (tnlxp[209] - client->character.XP, client);
									}*/
									//client->character.level = target - 1;
								}
								else
								{
									target -= 2;
			...
			if ( ( !strcmp ( myCommand, "sethp" ) ) && ((client->isgm) || (playerHasRights(client->guildcard, 7))) )
			{
				target = atoi ( myArgs[0] );

				if (target < 1 || target > 65535)
				{
					SendB0 ("M BERIBAAD broken value for hp m no gud", client);
				}
				else
				{
					SendB0 ("M berigud haxed ur HP", client);
					WriteGM ("GM %u (%s) has set HP to %u from (%u)", client->guildcard, Unicode_to_ASCII ((unsigned short*) &client->character.name[4]), target, client->character.HP);
					client->character.HP = target;
				}
			...
			/*//if (!strcmp(myCommand, "lobby"))
			{
				if (client->lobbyNum < 0x10)
					SendB0("Cannot warp in the lobby", client);
				else
				{
							warp_packet[0x0C] = (unsigned char) atoi ( myArgs[0] );
							//i was thinking of adding
							//SendToLobby (client->lobby, 12, &warp_packet[0], sizeof(warp_packet), client->guild_card - 1)
							//from the way i understand the function, it looks like it sends up to 12 players out of the lobby, except the player with the guild card number
							//i just wanted to assure that your guild card number wasn't 0!
							Packet66[0x08] = client->clientID
			...
			if (!strcmp(myCommand, "matcount"))
			{
				//send client popup window of matuse values
				char str[200];
			...
			Mind Materials : --
				Evade Materials : --
				Defense Materials : --
				Luck Materials : (equation) */

			//char format_str1[] = "client->matuse[0] = ( client->character.ATP - baseATP ) / 2\n"
			//                     "%d = (%d - %d) / 2\n\n";
			//char str[200];
			//sprintf(str, format_str1, (int)client->matuse[0], (int)client->character.ATP, (int)baseATP);
			//Send1A(str, client);

			if ( ( !strcmp ( myCommand, "warpall" ) )  && ((client->isgm) || (playerHasRights(client->guildcard, 10))) )
			{
				if ( client->lobbyNum < 0x10 )
					SendB0 ("hyYyyY M no gud warp in lobby M pls give gud deel first", client);
				else
					if ( myCmdArgs == 0 )
						SendB0 ("Need area to warp to...", client);
					else
...
void LoadArmorParam()
{
	...
	stackable_table[0x0A] = 99;
	stackable_table[0x0B] = 99;
}
